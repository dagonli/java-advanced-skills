...
import tecent.AudioBatchComparator;
import xunfei.iflytek.AudioBatchComparatorXunFei;
import xunfei.iflytek.AudioFormatUtils;
import xunfei.iflytek.VoiceprintRecognitionMain;
...
public class CreateFeature {
    ...
    //提供给主函数调用的方法
    public static JSONObject doCreateFeature(String requestUrl,String APPID,String apiSecret,String apiKey,String AUDIO_PATH){
    ...
    private String  buildParam() throws IOException {
        ...
        String featureId = file.getName().substring(0, file.getName().lastIndexOf("."));

        StringBuilder pinyin = new StringBuilder();

        for (char c : featureId.toCharArray()) {
            // 获取拼音，只取第一个发音（多音字情况）
            String[] pinyinArray = PinyinHelper.toHanyuPinyinStringArray(c);
            if (pinyinArray != null) {
                pinyin.append(pinyinArray[0]); // 取第一个发音
            } else {
                pinyin.append(c); // 非汉字字符直接保留
            }
        }
        String pinyinFeatureId = pinyin.toString();





        String featureInfo = pinyinFeatureId + (new Date().getTime());

        pinyinFeatureId = "DAGON_001";
        featureInfo = "DAGON_001_07302002";
        ...
        String param = "{"+
                ...
                "            \"func\": \"createFeature\","+
                //这里填上所需要的groupId
                "            \"groupId\": \""+ VoiceprintRecognitionMain.groupId +"\","+
                //这里填上所需要的featureId
                "            \"featureId\": \""+pinyinFeatureId+"\","+
                //这里填上所需要的featureInfo
...
//        in.close();
        ...
        byte[] wavData = Files.readAllBytes(new File("D:\\diyProject\\java-advanced-skills\\audio_compare_demo\\src\\main\\resources\\files\\李豫2.wav").toPath());
        byte[] bytes = null;
        bytes = AudioFormatUtils.wavToMp3(wavData);
